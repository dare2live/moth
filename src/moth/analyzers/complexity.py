...
def run(
    ...
    *,
    ...
    Scans one root (or several roots, concatenated) exactly like the CLI would
    and returns ``{"findings": [...], "total": N, "truncated": bool}`` where
    ``findings`` are JSON-able dicts in the frozen schema, sorted by severity
    rank and truncated to ``max_findings`` (CLI default: 80).
    """
    if isinstance(paths, (str, Path)):
        paths = [paths]
    effective_excludes = DEFAULT_EXCLUDES | {str(item) for item in excludes}
    findings: list[Finding] = []
    ...
    ordered = sorted(dedupe(findings), key=severity_rank)
    total = len(ordered)
    limited = ordered[:max_findings]
    return {
        "findings": [asdict(finding) for finding in limited],
        "total": total,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.format == "json":
        ...
        if result["total"] > len(findings):
            output += f"\nShowing {len(findings)} of {result['total']} findings. Raise --max-findings to see the rest.\n"
        print(output)
    return 0
